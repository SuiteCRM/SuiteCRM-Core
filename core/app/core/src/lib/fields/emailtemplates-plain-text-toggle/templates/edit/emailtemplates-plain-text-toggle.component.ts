...
@Component({
    selector: 'scrm-emailtemplates-plain-text-toggle-edit',
    templateUrl: './emailtemplates-plain-text-toggle.component.html',
    styles: [],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class EmailTemplatesPlainTextToggleEditFieldComponent extends BaseFieldComponent {
    ...
    get buttonLabel(): string {
        const key = this.isPlainTextVisible() ? 'LBL_HIDE_ALT_TEXT' : 'LBL_EDIT_ALT_TEXT';
        return this.language.getFieldLabel(key) || (this.isPlainTextVisible() ? 'Hide Plain Text' : 'Edit Plain Text');
    }
    ...
    private initTextOnlyMode(): void {
        ...
        this.textOnlyMode = this.toBoolean(textOnly?.formControl?.value ?? textOnly?.value ?? false);
    }

    private bindTextOnlyChanges(): void {
        const textOnly: any = this.record?.fields?.text_only;
        if (textOnly?.formControl?.valueChanges) {
            const formControlSub = textOnly.formControl.valueChanges.subscribe((value: any) => {
                this.textOnlyMode = this.toBoolean(value);
                this.applyVisibilityForCurrentModeWithRetry();
                this.cdr.markForCheck();
            });
            this.subs.push(formControlSub);
        }
